...
Pinning the URL to `main` means re-committing a file under the same name points at the new blob; pinning to a commit SHA instead freezes it. **But do not rely on that to swap an image that has already been published: once a raw URL has been fetched it stays cached, and re-committing different bytes under the same name leaves the article showing the old picture.** Replacing a figure means giving it a new filename — `part2-fig01.png` → `part2-fig01-v2.png` — and updating the reference in the article. This bites hardest after a renumbering: inserting a figure mid-article shifts every later `figNN`, so the *content* behind already-published URLs changes even though no name did. `git log --name-status -- images/<slug>/` shows which files were modified after being pushed; those are exactly the ones needing a new name. Article figures are flat-colour illustrations, so quantizing to a 256-colour palette (`convert in.png -strip -colors 256 -depth 8 PNG8:out.png`) cuts roughly 75% of the bytes with no visible loss — worth doing before committing, since these live in git forever.
