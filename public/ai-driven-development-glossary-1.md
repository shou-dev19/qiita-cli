...
tags:
  ...
  - 初心者
private: true
updated_at: '2026-08-03T21:27:07+09:00'
...
そういう人と話していると、だいたい同じところで止まっています。

> 「ネットの記事を読んでも、`Skills` とか `Subagent` とか `ハーネス` とか、**知らない単語が知らない単語で説明されてる**んだよね...」

これ、めちゃくちゃわかります。私も1年前は完全にそうでした。

そこで、その「知らない単語の壁」を崩すことだけに絞った記事を書くことにしました。前編・後編の2本立てでお届けします。

- **【前編】（この記事）**：歴史をたどりながら、用語が生まれた理由を理解する
- **【後編】**：2026年現在の最前線——ハーネスとオーケストレーション

### この記事のゴール（と、ゴールにしないこと）

**ゴール：AI駆動開発に出てくる用語の「概念」を、正しく理解すること。それだけ。**
...
![図1 Before / After](https://raw.githubusercontent.com/shou-dev19/qiita-cli/main/images/ai-driven-development-glossary-1/part1-fig01.png)
...
:::note warn
以下の時期は私の体感と記録ベースのざっくりした区分です。厳密なリリース日は公式のリリースノートをご確認ください。
:::
...
これ、**最初の1週間は最高に楽しい**んですよ。魔法みたいに動くものが出てくる。
...
新入社員に、就業規則もコーディング規約も渡さずに「いい感じにやっといて」と言っているのと同じ。そりゃそうなる。

### 第4世代：コンテキストエンジニアリング（2025年央）「記憶を持たせる」
...
![図6 コンテキストウィンドウ＝机の上](https://raw.githubusercontent.com/shou-dev19/qiita-cli/main/images/ai-driven-development-glossary-1/part1-fig06.png)
...
![図8 CLAUDE.md と Skills の違い](https://raw.githubusercontent.com/shou-dev19/qiita-cli/main/images/ai-driven-development-glossary-1/part1-fig08.png)
...
**Skills と Hooks の決定的な違い**：

| 用語 | 発動の仕方 |
|---|---|
| **Skills** | AIが「必要だ」と**判断したら**動く（＝気まぐれの余地がある） |
| **Hooks** | 条件が揃えば**必ず**動く（＝AIの判断を挟まない） |
...
![図10 MCP＝USBハブ](https://raw.githubusercontent.com/shou-dev19/qiita-cli/main/images/ai-driven-development-glossary-1/part1-fig10.png)

*図10｜AIとツール群をつなぐ共通規格*

:::note warn
MCPサーバーを繋ぎまくると、**ツールの定義だけで机の上が埋まります**（図6右）。「便利そうだから全部入れる」は典型的な失敗パターン。実際、この問題への対策として、ツール定義を必要になるまで読み込まない仕組み（MCP Tool Search）も用意されているくらいです。**繋ぐものは選ぶ。** ここは最初にやらかしやすいので、先に釘を刺しておきます。
:::
...
![図11 Subagent＝机を分ける](https://raw.githubusercontent.com/shou-dev19/qiita-cli/main/images/ai-driven-development-glossary-1/part1-fig11.png)
...
![図12 前編まとめ：困りごと→解決策の連鎖](https://raw.githubusercontent.com/shou-dev19/qiita-cli/main/images/ai-driven-development-glossary-1/part1-fig12.png)
...
あとから見返せるように、一覧にまとめておきます。ここだけブックマークしておくのもアリです。

#### 🧱 土台の用語

| 用語 | ひとことで | たとえると |
|---|---|---|
| **LLM / モデル** | 言語を扱う頭脳そのもの | 新入社員の地頭 |
| **エージェント** | ツールを使って自律的に動くAI | 手足を持った社員 |
| **ツール（Tool）** | AIが実行できる操作（読む・書く・実行する） | 社員に渡すPCと権限 |
| **エージェンティックループ** | 集める→動く→確かめる→繰り返す | 仕事の基本サイクル |
| **コンテキストウィンドウ** | AIの作業メモリ（有限） | 机の広さ |
| **コンパクション** | 溢れそうな文脈を要約して圧縮 | 議事録にまとめて書類を捨てる |
| **セッション** | 独立した1つの会話・作業単位 | 1日の勤務 |
| **バイブコーディング** | 雰囲気でAIに丸投げする開発スタイル | 指示書なしの丸投げ |

#### 📚 記憶と指示の用語

| 用語 | ひとことで | 使いどころ |
|---|---|---|
| **プロンプトエンジニアリング** | どう「聞く」かの技術 | 単発の依頼 |
| **コンテキストエンジニアリング** | 何を「渡す／渡さない」かの設計 | 継続的な開発全体 |
| **CLAUDE.md** | 毎回自動で読まれる指示書 | 常に必要な規約 |
| **Rules** | 条件付きで読み込まれる指示ファイル群 | 特定ファイルを触るときだけのルール |
| **Auto Memory** | AIが自分用に書き溜めるメモ | 学習内容の引き継ぎ |

#### 🧩 拡張・分業の用語

| 用語 | ひとことで | 発動タイミング／ポイント |
|---|---|---|
| **Skills** | 手順・知識をまとめた `SKILL.md` | AIが必要と判断したとき／`/名前`で指名 |
| **Hooks** | 特定タイミングで必ず走る処理 | **決定論的**（AIの判断を挟まない） |
| **MCP** | 外部サービス接続のオープン標準 | ツールを増やしたいとき |
| **Plugin** | Skills・Hooks・Subagents・MCPの詰め合わせ | チームへの配布 |
| **Command（`/xxx`）** | 名前で呼び出す定型指示 | 現在は Skills が推奨後継 |
| **Subagent** | 別コンテキストで働く委任先 | **机が別**。要約だけ返る |
...
この問いに答えるのが、2026年現在のホットトピック——**ハーネス**と**オーケストレーション**です。
