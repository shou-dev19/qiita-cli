...
tags:
  ...
  - 初心者
private: true
updated_at: '2026-08-07T22:12:48+09:00'
...
**内側で勝手に回っている歯車がエージェンティックループ、その外側に人間が被せる運用の輪がいわゆるループエンジニアリング。** 言葉は似ていますが階層が違います。**外側の輪については記事の後半（「♾ ループエンジニアリング」）で改めて扱う**ので、ここでは階層が違うことだけ押さえてください（それまで単に「ループ」と言うときは、内側のほうを指します）。
:::
...
検証ループは、AIが**自分で合否を判定できる**仕組みでした。では、合否を自分で判定できるようになったAIに対して、人間はまだ何を渡す必要があるでしょうか。

ほとんど「**次の一周を始めていいよ**」だけです。**それなら人間が渡さなくてもいいのでは？** ——この発想が、最近よく聞くループエンジニアリングです。
...
| 輪 | 誰が回すか | 一周の長さ | 止まるとき |
|---|---|---|---|
| **エージェンティックループ**<br>（内側） | エージェントが勝手に | 数秒〜数分 | 依頼を1つこなしたとき |
| **検証ループ**<br>（真ん中） | テストが合否を返す | 数分〜数十分 | 合格したとき |
| **ループエンジニアリング**<br>（外側） | **人間が設計する** | 数時間〜一晩 | **停止条件を満たしたとき** |

> 🖼 【図4 3重の輪】
> 同心円を3つ描く。内側から「エージェンティックループ（AIが自動で回す）」「検証ループ（テストが合否を返す）」「ループエンジニアリング（人間が設計する）」。
> 内側2つの円には歯車のアイコン、いちばん外側の円だけ人間のアイコンを添えて、「ここだけが人間の仕事」とわかるようにする。
> キャプション：「内側は自動で回る。人間が設計するのは、いちばん外の輪」

### 人間は、どこでボトルネックになっていたのか

これまでは、AIが一周終えるたびに人間のところへ戻ってきていました。「できました！」「OK、じゃあ次はこれ」——この往復です。
...
（講演でのコメントがクリップとして広まったもので、文字起こしは媒体によって少しずつ違います。上記は趣旨をまとめたものです。英語での表現や背景は [Addy Osmani, "Loop Engineering"](https://addyosmani.com/blog/loop-engineering/) が読みやすいです）

**「ループエンジニアリング」という言葉が一気に広まったのは、この発言がきっかけ**でした。私が知ったのもここからです。

正直、最初に読んだときは「さすがに言い切るなあ」と思いました。でも自分でハーネスを作り込んでいくと、確かに同じ方向に足が向いていくんですよね。だから私も、**いまエンジニアの仕事は「AIにループを回させられるハーネスを作ること」になりつつある**と思っています。冒頭で「脳汁ドバドバ」と書いたのは、まさにこの部分です。<!-- TODO: ここに実体験の数字を1つ入れる（例：一晩で何周回ったか／朝どれだけ積み上がっていたか） -->
...
私が実際にやってみて、いちばん割に合ったのは **「まず5〜7割をサッと自動化して、残りは手でやる」** でした。理由が3つあります。
...
そしてこれが大きい。**モデルが賢くなるスピードのほうが、自分が仕組みを作り込むスピードより速い。** 苦労して書いた例外処理が、次のモデルでは「何もしなくても普通に処理される」ようになる。この1年で何度も見た光景です。
...
つまり、**「精密なループを組み上げる」という方向性自体が、少し古くなりつつある**わけです。渡すのはゴールと合格基準。やり方はモデルに任せる。
...
実際に何をどう仕込むのか——タスクの切り方、停止条件の書き方、朝いちで成果物をどう受け取るか、動画ワークフローで人間のチェックをどこに置いているか——は、この記事の役割（用語の壁を崩す）を超えるので、また別の機会に書きます。

ここでは **「そういう営みに名前がついていて、それは検証ループの外側の話だ」**、そして **「最初から全部を自動化しようとしなくていい」** の2つだけ持ち帰ってください。
...
自動化も同じです。**最初から全部を自動化しようとしなくていい。** 5割から始めて、辛いところから順に埋めていけば十分です。
...
| **ループエンジニアリング** | 停止条件まで人手なしで回し続ける運用設計 | 検証ループの**外側**の輪。順番を逆にしない |
