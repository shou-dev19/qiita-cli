...
だから私は、**いまエンジニアの仕事は「AIにループを回させられるハーネスを作ること」になった**と思っています。冒頭で「脳汁ドバドバ」と書いたのは、まさにこの部分です。<!-- TODO: ここに実体験の数字を1つ入れる（例：一晩で何周回ったか／朝どれだけ積み上がっていたか） -->
...
実際に何をどう仕込むのか——タスクの切り方、停止条件の書き方、朝いちで成果物をどう受け取るか——は、この記事の役割（用語の壁を崩す）を超えるので別記事にします。ここでは **「そういう営みに名前がついていて、それは検証ループの外側の話だ」** とだけ持ち帰ってください。
