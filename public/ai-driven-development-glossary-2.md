...
社内でAI駆動開発の勉強会を頼まれたので、その資料を兼ねて書いている記事の**後編**です。

対象は「IT業界で働いていて、プログラミングはできる。でもAIエージェントを使った開発の知見はあまりない」という人。**Skills？ オーケストレーター？ ハーネス？ ——という状態からの脱出**がゴールです。
...
| **エージェンティックループ** | 集める→動く→確かめる→繰り返す、というエージェントの基本サイクル |
...
1. **詳細は必ず公式で。** この記事は地図であって辞書ではありません（リンクは末尾に）
...
「AIが賢い／賢くない」という議論は、たいてい**中心（モデル）の話だけ**をしています。でも実際の成果を決めているのは、その外側の2層です。ここが分かれ目。
...
| プロンプトを工夫する | ルールを Hooks に落とし、手順を Skills に切り出す |
...
正直に白状すると、私はいまここに完全にハマっています。**いかに適切なハーネスを作り込み、いかにAIにループを回させ、人間が介在するポイントを減らしながら品質を保つか。** これを考えている時間がいちばん楽しい。ドーパミンが出ます。

コードを書く快感とは違う、**「仕組みが自動で回り始める快感」**があるんですよね。夜、寝る前に仕込んだループが、朝起きたら成果物を積み上げている。あれは中毒性があります。
...
人間だってそうです。受け入れ基準のない仕事は「まあ、できたかな」で終わります。AIも同じ。「できました！」と自信満々に言ってくるけど動かない、というアレの正体はこれです。

逆に、テストという合格基準を渡せば、**AIは通るまで自分で直し続けます**。これがループの燃料になる。
...
**特に2つ目が致命的です。** 自分の答案を自分で採点させたら、そりゃ甘くなる。人間でも同じです。
...
ここまで「いかに人間の手を減らすか」を語ってきましたが、**その裏返しの話**を必ずセットで押さえてください。勉強会でも必ず触れるつもりのパートです。
...
![図9 全体マップ（前編・後編の総まとめ）](https://raw.githubusercontent.com/shou-dev19/qiita-cli/main/images/ai-driven-development-glossary-2/part2-fig09.png)

*図9｜前編・後編で登場した用語の全体マップ*
...
ただ、**地図だけは先に持っておいたほうがいい。** それが遠回りを減らします。この2記事がその地図になれば嬉しいです。
...
「ここの理解が違うのでは」「この用語も入れてほしい」というご意見があれば、ぜひコメントで教えてください。勉強会の資料に反映させていただきます 🙏
